...
/*
   ...
   printLog ("! GMUSTA1 only works in Cartesian coordinates\n");
   QUIT_PLUTO(1);
  #endif
*/
  if (f_LF == NULL){
    printLog (" >> GMUSTA1 Solver\n");
...
for (i = beg; i <= end; i++) {
  NFLX_LOOP(nv) {
    sweep->flux[i][nv] =    om*stateLW.flux[i][nv]
                          + (1.0 - om)*f_LF[i][nv];
  }
  sweep->press[i] = 0.0;
}
...
/* ----------------------------------------------
  ...
  #if (defined STAGGERED_MHD) && (CT_EMF_AVERAGE == UCT_VELOCITY)
  for (i = beg; i <= end; i++){
    sweep->pnt_flux[i][BXt] = 0.5*(fL[i][BXt] + fR[i][BXt]);
    sweep->pnt_flux[i][BXb] = 0.5*(fL[i][BXb] + fR[i][BXb]);
    sweep->dff_flux[i][BXt] = sweep->flux[i][BXt] - sweep->pnt_flux[i][BXt];
    sweep->dff_flux[i][BXb] = sweep->flux[i][BXb] - sweep->pnt_flux[i][BXb];
  }
  #endif
